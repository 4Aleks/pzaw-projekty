...
import "dotenv/config"; // Ładuje plik .env

const PEPPER = process.env.PEPPER;

if (PEPPER == null) {
  console.error("PEPPER environment variable missing.");
  process.exit(1);
}
...
const db_ops = {
  create_user: db.prepare("INSERT INTO fc_users (username, passhash, created_at) VALUES (?, ?, ?) RETURNING id;"),
  get_user: db.prepare("SELECT id, username, created_at FROM fc_users WHERE id = ?;"),
  find_by_username: db.prepare("SELECT id, username, created_at FROM fc_users WHERE username = ?;"),
  get_auth_data: db.prepare("SELECT id, passhash FROM fc_users WHERE username = ?;"),
};
...
export default { createUser, validatePassword, getUser };